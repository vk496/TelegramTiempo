...
print_tiempo() {
    ...
    page=$(curl -qsL https://www.eltiempo.es/$lugar.html | xmlstarlet fo -H -D 2>/dev/null | xpathc "//article[@data-next-week]//div[@data-next-week-slider]" )
...
function send(){
    ...
    for i in "${to[@]}"; do
        (sleep 5; echo "safe_quit") | eval $TG_CLI -U root -G root -W -D -e \"msg $i \'$mensaje\'\"
    done
